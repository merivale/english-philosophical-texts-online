...
function formatLemma (lemma) {
  return lemma.replace(/_\b/g, '') // get rid of disambiguating underscores at the ends of words
...
function people (blocks) {
  return blocks.map(block => block.content.match(/<u>.*?<\/u>/g))
    .reduce((x, y) => x.concat(y), []).filter(Boolean)
    .map(x => x.replace(/<a(.*?)>(.*?)<\/a>/g, '').replace(/<([^>]+)>/g, '')) // strip html
...
function citations (blocks) {
  return blocks.map(block => block.content.match(/<cite>.*?<\/cite>/g))
    .reduce((x, y) => x.concat(y), []).filter(Boolean)
    .map(x => x.replace(/<a(.*?)>(.*?)<\/a>/g, '').replace(/<([^>]+)>/g, '')) // strip html
}

// get foreign text from blocks
function foreign (blocks) {
  return blocks.map(block => block.content.match(/<i>.*?<\/i>/g))
    .reduce((x, y) => x.concat(y), []).filter(Boolean)
    .map(x => x.replace(/<a(.*?)>(.*?)<\/a>/g, '').replace(/<([^>]+)>/g, '')) // strip html
}
